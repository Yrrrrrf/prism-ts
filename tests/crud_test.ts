...
import { PrismTs, BaseClient, createCrudOperations } from "../src/mod.ts";
...
function setupMockFetch() {
  ...
  return {
    addMock: (method: string, endpoint: string, data: unknown, status = 200) => {
...
Deno.test("TableOperations - CRUD methods", async () => {
  ...
  try {
    ...
    // Test update
    const updatedUser = await userOps.update(userId, { email: "updated@example.com" });
