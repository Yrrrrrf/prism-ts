...
async function testColumnInspection(context: ViewTestContext): Promise<void> {
  ...
  try {
    if (!context.viewMetadata.columns || context.viewMetadata.columns.length === 0) {
...
async function testDataAnalysis(context: ViewTestContext): Promise<void> {
  ...
  try {
    ...
    sampleRecords.forEach((record: { [s: string]: unknown; } | ArrayLike<unknown>) => {
    ...
    nullCounts.forEach((count, col) => {
      ...
      const nullPercent = Math.round((count / sampleRecords.length) * 100);
      log.info(`  ${col.padEnd(20)}: ${uniqueCount} unique values, ${nullPercent}% null`);
    });
    ...
    if (columns.length > 0) {
      ...
      sampleRecords.forEach((record: { [x: string]: any; }) => {
        const value = record[sampleColumn] === null ? 'null' : String(record[sampleColumn]);
        valueCounts.set(value, (valueCounts.get(value) || 0) + 1);
      });
      
      // Display distribution
      valueCounts.forEach((count, value) => {
        const percent = Math.round((count / sampleRecords.length) * 100);
...
async function runViewTests(): Promise<void> {
  ...
  try {
    ...
    log.info(`Successfully tested view: ${cyan(context.schemaName)}.${green(context.viewName)}`);
    log.info("All view tests completed");
    
    // Provide recommendations for fixing server issues
    log.section("Server-Side Issues Detected");
    log.info(`${yellow("⚠️")} The following issues were detected with the Prism server:`);
    log.info("");
    log.info(`${red("1.")} View filtering has a bug in the Prism Python library.`);
    log.info("   Error: TypeError: Boolean value of this clause is not defined");
    log.info("   Location: prism/api/views.py, line 63");
    log.info("   Fix: Change 'if self.table and field_name in self.table.columns:' to");
    log.info("        'if hasattr(self, \"table\") and self.table is not None and field_name in self.table.columns:'");
    log.info("");
    log.info(`${red("2.")} View pagination may not be working correctly.`);
    log.info("   The 'offset' parameter might be ignored in the server implementation.");
    log.info("");
    log.info("These issues should be fixed in the Prism Python library for full view functionality.");
  } catch (error) {
    log.error(
      `Test suite failed: ${
        error instanceof Error ? error.message : String(error)
